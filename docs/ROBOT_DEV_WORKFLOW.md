...
```bash
# SSH with X11 forwarding (always enabled for GUI apps)
robopi
```
...
2. Use `robopi` - X11 forwarding is always enabled
...
**Example:**
```bash
robopi                      # SSH with X11 (always enabled)
...
robopi         # SSH into robot (with X11 forwarding for GUI apps)
